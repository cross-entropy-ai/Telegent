...
func settingsItems(data: PeerInfoScreenData?, context: AccountContext, presentationData: PresentationData, interaction: PeerInfoInteraction, isExpanded: Bool) -> [(AnyHashable, [PeerInfoScreenItem])] {
    ...
    items[.advanced]!.append(PeerInfoScreenDisclosureItem(id: 0, label: .none, text: presentationData.strings.Settings_NotificationsAndSounds, icon: PresentationResourcesSettings.notifications, action: {
    ...
    items[.advanced]!.append(PeerInfoScreenDisclosureItem(id: 7, text: "Telegent", icon: PresentationResourcesSettings.support, action: {
        interaction.openSettings(.telegent)
    }))
    ...
    // Telegent: My Stars and Send a Gift removed
    ...
    // Telegent: Ask a Question, FAQ, Tips removed
